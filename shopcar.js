$(document).ready(function() {
    ...
    // 只要options_box出现在可视区域内，就让固定定位在底部的盒子隐藏，否则显示——>如何实现？
    var options_box = document.querySelector('.options-box');
    var options_boxTop = options_box.offsetTop; // 1052
    window.onscroll = function() {
        console.log(document.body.clientHeight, document.documentElement.clientHeight, options_boxTop);
        // 只适用于商品未被删除的情况，要做动态调整
        /*  if (window.pageYOffset <= 338) {
             options_box.style.position = 'fixed';
             options_box.style.bottom = 0;
             options_box.style.left = '159.6px';
             options_box.style.margin = 0;
        ...
        if (options_boxTop >= document.documentElement.clientHeight) {
            ...
            options_box.style.margin = 0;
            options_box.style.backgroundColor = '#f3f3f3';
        } else if (options_boxTop < document.documentElement.clientHeight) {
            options_box.style.position = '';
            options_box.style.bottom = '';
            options_box.style.left = '';
            options_box.style.margin = '20px auto';
